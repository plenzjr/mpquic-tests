...
from core.generate_topo import generateTopoFile, PATHS, DELAY, QUEUE_SIZE, QUEUING_DELAY, BANDWIDTH, LOSS, NETEM
from core.generate_xp import generateXpFile
...
def topoToFilename(topo):
    ...
    for netem in topo.get(NETEM, []):
        toReturn += "_nt_" + \
            str(netem[0]) + "_" + str(netem[1]) + \
            "_" + str(netem[2]).replace(" ", "_")
...
class ExperienceLauncher(object):
    ...
    def __init__(self, remoteHostnames, remotePorts):
        ...
        if not len(self.remoteHostnames) == len(self.remotePorts):
            raise Exception(
                "remoteHostnames and remotePorts with different lengths")
    ...
    def putOnRemote(self, num, filename, path):
        cmd = "scp -P " + self.remotePorts[num] + " " + \
            filename + " " + self.remoteHostnames[num] + ":" + path
        if subprocess.call(cmd.split()) != 0:
            raise Exception("File " + filename +
                            " could not be put on remote server at path " + path)

    def pullHereFromRemote(self, num, filename, path, newFilename):
        cmd = "scp -P " + self.remotePorts[num] + " " + \
            self.remoteHostnames[num] + ":" + path + \
            "/" + filename + " " + newFilename
        if subprocess.call(cmd.split()) != 0:
            raise Exception(
                "File " + filename + " could not be pull from remote server at path " + path)

    def changeMptcpEnabled(self, num, value):
        cmd = ["ssh", "-p", self.remotePorts[num], self.remoteHostnames[num],
               "sudo sysctl net.mptcp.mptcp_enabled=" + str(value)]
        if subprocess.call(cmd) != 0:
            raise Exception(
                "Cannot change value of mptcp_enabled at " + str(value))
    ...
    def postProcessing(self, num, **kwargs):
        ...
        for postProcess in kwargs["postProcessing"]:
            if len(postProcess) == 2:
                ...
                raise Exception(
                    "Invalid number of elements in postProcessing: " + str(postProcess))

            self.pullHereFromRemote(num, remoteFilename, remotePath, os.path.join(
                kwargs["workingDir"], localFilename))

    def cleanMininet(self, num):
        cmd = ["ssh", "-p", self.remotePorts[num],
               self.remoteHostnames[num], "timeout 20 sudo mn -c"]
        devnull = open(os.devnull, 'w')
    ...
    def launchXp(self, num, **kwargs):
        ...
        MinitopoCommand(num, self.remoteHostnames[num], self.remotePorts[num],
                        cmd, kwargs["workingDir"], self.testOkList).run(timeout=THREAD_TIMEOUT)
...
def experimentTopos(topos, xpName, protocol, tmpfs, xpFnct, **kwargs):
    testDirectory = xpName + "_" + \
        time.strftime("%Y%m%d_%H%M%S") + "_" + protocol
    ...
    for topo in topos:
        ...
        xpFnct(xpName=xpName, testDirectory=testDirectory, topoAbsPath=topoAbsPath,
               protocol=protocol, topo=topo, tmpfs=tmpfs, **kwargs)
